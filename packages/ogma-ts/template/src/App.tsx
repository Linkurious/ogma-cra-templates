...
import Ogma, { HoverData } from "./components/Ogma";
import Tooltip from "./components/Tooltip";
import { NodeId, Point, RawEdge, RawGraph } from "@linkurious/ogma";
...
function App() {
  const [graph, setGraph] = useState<RawGraph<NodeData>>({
    nodes: [
      { id: 0, data: { name: "A" } },
      { id: 1, data: { name: "B" } },
    ],
  ...
  const addNode = useCallback(() => {
    ...
    setGraph({
      ...
      edges: [
        ...graph.edges,
        { source: lastNode.id, target: graph.nodes.length } as RawEdge,
      ],
    });
  ...
  function onNodeHover(target: HoverData<NodeData, any> | null) {
    if (target) {
